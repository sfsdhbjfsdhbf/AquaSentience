...
import random
from ..models.gailvzengqiang import ExplicitLowFreqAttention
import torch
...
class Wam(nn.Module):
    ...
    def __init__(
        ...
        self.enhance_pro = ExplicitLowFreqAttention(256,256)
    ...
    def forward(
        ...
        # 获得每个水印区域的msg
            ...
            # 增大低频水印 尽量抑制其他
            deltas_w = self.enhance_pro(deltas_w)

            imgs_w = self.scaling_i * imgs + self.scaling_w * inverse_resize(deltas_w)
            if self.attenuation is not None:
                imgs_w = self.attenuation(imgs, imgs_w)
            # combine_imgs和 imgs_w有什么区别？imgs_w 是当前水印区域的水印图像 combine是总体的，注意combine是累加的
            if not roll:
