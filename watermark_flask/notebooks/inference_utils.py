...
def create_weird_shape_mask(img_pt,height, width):
    """
    创建一个奇怪的形状的mask
    Args:
    - height (int): mask的高度
    ...
    for i in range(height):
        for j in range(width):
            # 随机选择条件来决定哪些区域属于形状的一部分
            if (np.sin(i / 10) + np.cos(j / 10)) > 0.5 and (np.random.rand() > 0.7):
                mask[i, j] = 1

    return torch.tensor(mask, dtype=torch.float32).to(img_pt.device)
