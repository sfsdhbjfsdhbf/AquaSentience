...
# 项目根目录（当前已将模型文件、configs、水印包放在根目录）
from flask import send_file

_ROOT_DIR = os.path.abspath(os.path.dirname(__file__))
_WAM_DIR = _ROOT_DIR  # 直接使用根目录作为工作目录

# 保障能找到 watermark_anything 包
...
def _import_external_pkg(pkg_name: str):
    """
    如果本地存在与第三方同名的目录（例如本项目的 flask/ 目录），会遮蔽 site-packages。
    这里临时移除当前项目路径，确保导入的是真正的第三方包。
    """
    local_dirs = {
        os.path.abspath(os.path.join(_ROOT_DIR, "flask")),  # 旧目录
        _ROOT_DIR,
        "",  # 有些情况下 CWD 以空串形式出现
    }
    removed = []
...
# 引入 Flask / CORS
flask_pkg = _import_external_pkg("flask")
...
def load_model_once():
    global wam
    logger.info("===> 加载模型中...")
    # inference_utils 的相对路径基于 watermark-anything-main
    old_cwd = os.getcwd()
    try:
        os.chdir(_WAM_DIR)
...
def bits_str_to_tensor(bits: str) -> torch.Tensor:
    bits = bits[:32].ljust(32, "0")
    base_msg = wam.get_random_msg(1).to(device)  # 用于 dtype/device 对齐
...
def encode_full():
    ...
    return jsonify({
        ...
        "download_url": f"/download?file={watermarked_image_url.lstrip('/')}",
    })
...
def download():
    ...
    path = os.path.join(_ROOT_DIR, file)
    if not os.path.exists(path):
        return jsonify({"error": "file not found"}), 404
    # 如果需要限制目录，可再校验 path 是否在 WAM_OUT_DIR 下
...
def decode():
    ...
    mask_pre_url = f"/static/mask_pre/{mask_pre_name}"


    return jsonify({
        "decoded_bits_32": pred_bits_str,
        "decoded_msg_text": decoded_text,
        "pred_mask_url": mask_pre_url ,
        "original_image_url": f"/static/ori/{ori_name}",
        "download_url": f"/download?file={mask_pre_url.lstrip('/')}",
    })
